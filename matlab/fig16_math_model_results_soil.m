load(fullfile('data', 'scarf_permittivity_soil.mat'))
load(fullfile('data', 'lightness_cali_soil.mat'))
load(fullfile('data', 'oven_vwc_data_soil.mat'))
addpath('utils/')
...
e_measure_all = scarf_result_soil + 2;
l_measure_all = lightness_soil_cali;
moisture_levels = oven_vwc_soil;
...
% OC
bulk_density_soil = [1.0380    1.0200    1.0160    0.9830];
OC_soil = [1.3456    2.2976    3.1541    3.8787];
OC_measured_2d = reshape(repmat(OC_soil, 1, size(e_measure_all, 2)), [], size(e_measure_all, 2));
p = polyfit(OC_soil, bulk_density_soil,1);
b = p(1); 
a = p(2);
...
bulk_density_soil = b * OC_range + a;
p = 0.6819 - 0.0648 ./ (OC_range + 1) - 0.119 * bulk_density_soil.^2 - 0.02668 ...
    + 0.1489 * v_clay + 0.08031 * v_silt + 0.02321 ./ ((OC_range + 1) .* (bulk_density_soil.^2)) ...
    - 0.01908 * bulk_density_soil.^2 - 0.1109 * v_clay - 0.2315 * v_clay * v_silt ...
    - 0.01197 * v_silt* bulk_density_soil.^2 - 0.01068 * v_clay * bulk_density_soil.^2;
...
estimated_w = [];
for box_id = 1:4
    for moisutre_id = 1:size(e_measure_all,2)
        l_measure = l_measure_all(box_id, moisutre_id);
        e_measure = e_measure_all(box_id, moisutre_id);
        w = (-30.1865*log(OC_range+1) + 133.2787 - l_measure)/2.5665 / 100; % calibrated lightness      
        e_a = [];
        for i = 1:length(w)
            e_a(i) = compute_e_soil(OC_range(i), w(i), e_soil, e_bound, e_air, e_free); 
        end
        obj_fun = abs(e_a - e_measure);
        [val, id] = min(obj_fun);
        estimated_oc(box_id, moisutre_id) = OC_range(id);
        estimated_w(box_id, moisutre_id) = w(id);
    end
end
